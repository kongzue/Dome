...
import com.kongzue.dome.BuildConfig;
...
/**
 * Created by ZhangChao on 2017/6/9.
 */
...
public abstract class BaseActivity extends Activity {
    ...
    public static double dip2pxToDouble(Context context, float dpValue) {
        final float scale = context.getResources().getDisplayMetrics().density;
...
//        log("StatusBarHeight:" + (int) getStatusBarHeight());
    ...
    //动态刷新列表高度
    public void refreshListViewHeight(ListView listView, int itemHeightInDp) {
        if (itemHeightInDp == 0) itemHeightInDp = 65;
        int listViewHeight = 0;
        int adaptCount = listView.getAdapter().getCount();
        listViewHeight = dip2px(me, itemHeightInDp + 0.2f) * adaptCount - dip2px(me, 0.2f);
